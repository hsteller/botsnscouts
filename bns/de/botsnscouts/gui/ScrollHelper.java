...
public class ScrollHelper {
    ...
    /**
     * This method is a helper method to calculate a list of points on a
     * straight line between two points.
     * 
     * @param startpoint
     *            One of the endpoints of the straight line
     * @param endpoint
     *            The other endpoint of the straight line
     * @param offset
     *            the maximum number of pixels to move in one step (horizontal
     *            or vertical)
     * @return a list of Points between startpoint and endpoint; the last entry
     *         will alway be a copy of <code>endpoint</code>
     */
    public static Point[] calculateDiagonalScrollSteps(Point startpoint, Point endpoint, int offset) {
        ...
        // there is the problem of Java having (0,0) in the upper left of the
        // screen
        ...
        boolean isYinc = ydistance >= 0; // check if we increase y (==if we
                                         // scroll down)
        boolean isXinc = xdistance >= 0; // check if we increase x (==if we
                                         // scroll to the right)

        // depending on the moving directions we will either have to add or
        // subtract "offset" to the coordinates of "startpoint" to get to
        // "endpoint"
        // (or not changing one of them at all if we only scroll horizontal XOR
        // only vertical)
        ...
        // checking whether the horizontal or the vertical scrolling distance is
        // greater:
        // (needed, because we have to calculate the number of steps for the
        // maximum
        ...
        // rounding up, because otherwise the last step might be greater than
        // offset;
        ...
        if (numOfSteps == 0) { // hack: hatte eine IndexOutOfBoundsEx mit
                               // scrollPoints[-1]
            return new Point[] { new Point(endpoint) };
        }

        // end of the loop over the steps; is one less than numOfSteps because
        // as coordinates
        ...
        if (xdistance == 0) { // an easy special case: we move only vertical
            // => we can't (and don't need to) use the line equation below as
            // "m" (see below)
            // would be infinite
            for (int i = 0; i < end; i++) {
                y += verticalOffset;
        ...
        // we will now create the equation/function for the straight line
        // between newUpperLeft and
        // currentUpperLeft as we need to find the closest y-value to any
        // x-value between
        ...
        // newUpperLeft := (c,d)
        //
        float bMinusD = -ydistance; // (already calculated above for finding the
                                    // max distance)
        float aMinusC = -xdistance; // (already calculated above for finding the
                                    // max distance)
        float m = bMinusD / aMinusC; // slope of the line (german: Steigung der
                                     // Gerade)
        // n (the y-axis intercept is zero because of the above transformation
        // of
        // "startpoint" to the point of origin (0,0))

        if (xIsLonger) { // "sane" case: we move along the x-axis and use the
                         // line equation
            ...
            for (int i = 0; i < end; i++) {
                ...
                y = (int) (m * x);
                if (isXinc != isYinc) {
                    // this is the fix for Java having the point of origin in
                    // the upper left corner
                    y = -y;
                }
                // creating the new intermediate scrollpoint, undo of the
                // startpoint->(0,0) transformation
                scrollPoints[i] = new Point(x + transX, y + transY);
            }
        }
        else { // "not-so-sane-case":
               // we need to calculate the inverse function because we use the
               // ydistance and need
            ...
            for (int i = 0; i < end; i++) {
                ...
                if (isXinc != isYinc) {
                    // this is the fix for Java having the point of origin in
                    // the upper left corner;
                    // here we have to fix the x-coordinate because we are
    ...
    /**
     * Calculates intermediate points for scrolling from <code>currentUpperLeft</code> to <code>newUpperLeft</code>. The scrolling
     * is done in a right-angle: we either scroll first vertical, then
     * horizontal or first horizontal, then vertical.
     * 
     * @param newUpperLeft
     *            endpoint of scrolling; last point in returned array
     * @param currentUpperLeft
     *            startpoint of scrolling
     * @param offset
     *            maximum amount to scroll per step
     * @param horizontalFirst
     *            toggles whether order of scroll direction is
     *            horizontal->vertical or vertical->horizontal
     */
    public static Point[] scrollRightAngled(Point newUpperLeft, Point currentUpperLeft, int offset,
                    boolean horizontalFirst) {
        Collection<Point> horizontalScrollPoints = null;
        Collection<Point> verticalScrollPoints = null;
        ...
        Iterator<Point> firstDirection;
        Iterator<Point> sndDirection;
        if (horizontalFirst) {
        ...
        while (firstDirection.hasNext()) {
            retour[counter++] = firstDirection.next();
        }
        while (sndDirection.hasNext()) {
            retour[counter++] = sndDirection.next();
        }
        return retour;
    }

    private static Collection<Point> scrollWest(Point cur, Point pref, int diff) {
    ...
    private static Collection<Point> scrollEast(Point cur, Point pref, int diff) {
        int stop = pref.x - diff;
        LinkedList<Point> pointList = new LinkedList<Point>();
        while (stop > cur.x) {
            cur.x += diff;
    ...
    private static Collection<Point> scrollSouth(Point cur, Point pref, int diff) {
    ...
    private static Collection<Point> scrollNorth(Point cur, Point pref, int diff) {
        int stop = pref.y + diff;
        LinkedList<Point> pointList = new LinkedList<Point>();
        while (stop < cur.y) {
            cur.y -= diff;
    ...
    public static void main(String[] args) {
        ...
        Point[] testPoints = new Point[] { targetLeft, targetRight, targetUp, targetDown, targetUpperLeftXLonger,
                targetUpperRightXLonger, targetDownLeftXLonger, targetDownRightXLonger, targetUpperLeftYLonger,
                targetUpperRightYLonger, targetDownLeftYLonger, targetDownRightYLonger };
