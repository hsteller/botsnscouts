...
public class AnimationsSettingEditor extends JFrame {
    ...
    private static final Category CAT = Category.getInstance(AnimationsSettingEditor.class);

    private static final String msgSec = "AnimationSettings";

    protected final static TJLabel labelSpeedSlow = new TJLabel(Message.say("AusgabeFrame", "mSlow"),
                    SwingConstants.CENTER);

    protected final static TJLabel labelSpeedMedium = new TJLabel(Message.say("AusgabeFrame", "mMiddle"),
                    SwingConstants.CENTER);

    protected final static TJLabel labelSpeedFast = new TJLabel(Message.say("AusgabeFrame", "mFast"),
                    SwingConstants.CENTER);
    ...
    /**
     * If you change this array you also have to change #EditorPanel.textFields
     * so that this Labelarray still matches the Textfieldarray of EditorPanel
     * (both arrays must have the same order).
     */
    private static final JLabel[] SETTING_NAMES = new JLabel[] { labelMoveOffset, labelMoveDelay, labelTurnSteps,
            labelTurnDelayPerStep, labelLaserSoundBeforeAnimations, labelLaserDelayPerStep,
            labelLaserDelayAfterAnimation, labelDelayBetweenActions, labelDelayRevealingCards, labelDelayCardHighlight };
    ...
    public AnimationsSettingEditor(AnimationConfig slow, AnimationConfig medium, AnimationConfig fast) {
        ...
        // ColoredPanel(GreenTheme.getBnsBackgroundColor()));
    ...
    public class EditorPanel {
        ...
        /**
         * If you change this array you also have to change
         * #AnimationSettingsEditor.SETTING_NAMES because this JTextField-Array
         * has to match the JLabel-Array SETTING_NAMES.
         */
        private TJTextField[] textFields;

        /**
         * Contains Textfields that will set new values to the underlying
         * animation config. To match the names of the speeds in the speed menu
         * one should use AnimationsSettingEditor.labelSpeedXYZ as the second
         * parameter
         * 
         * @param conf
         *            The actual settings that should be edited by this panel
         * @param configName
         *            The name of these settings; should match the speed menu.
         * 
         */
        public EditorPanel(AnimationConfig conf, TJLabel configName) {
            ...
            laserDelayBetweenStartOfSoundAndAnimation = new TJTextField(""
                            + conf.getLaserDelayBetweenStartOfSoundAndAnimation(), 6);
            ...
            // If you change this array you also have to change
            // #AnimationSettingsEditor.SETTING_NAMES because
            // this JTextField-Array has to match the JLabel-Array
            // SETTING_NAMES.

            textFields = new TJTextField[] { animationOffsetMoveRob, animationDelayMoveRob, animationStepsTurnRob,
                    animationDelayTurnRob, laserDelayBetweenStartOfSoundAndAnimation, laserDelayPerAnimationStep,
                    laserDelayAfterEndOfAnimation, delayBetweenActions, delayAfterRevealingCardsForPhase,
                    delayAfterCardHighlight };
        }
        ...
        protected void applyValues() {
            int moveRob = getIntValue(animationOffsetMoveRob, 0, Integer.MAX_VALUE, config.getAnimationOffsetMoveRob());
            config.setAnimationOffsetMoveRob(moveRob);

            int moveDelay = getIntValue(animationDelayMoveRob, 0, 5000, config.getAnimationDelayMoveRob());
            config.setAnimationDelayMoveRob(moveDelay);

            int turnSteps = getIntValue(animationStepsTurnRob, 1, 90, config.getAnimationStepsTurnRob());
            config.setAnimationStepsTurnRob(turnSteps);

            int turnDelay = getIntValue(animationDelayTurnRob, 0, 2000, config.getAnimationDelayTurnRob());
            ...
            int laserDelay = getIntValue(laserDelayPerAnimationStep, 0, 5000, config.getLaserDelayPerAnimationStep());
            ...
            int actionDelay = getIntValue(delayBetweenActions, 0, 5000, config.getDelayBetweenActions());
