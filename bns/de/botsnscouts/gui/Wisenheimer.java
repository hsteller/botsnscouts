...
public class Wisenheimer {
     ...
     * 
     * @param cards
     *            ArrayList of HumanCards we can use for this move
     * @param registers
     *            ArrayList of HumanCards that are already programmed
     * @param robi
     *            current values of the bot we want to do the oredicition for
     */

    protected int getPrediction(ArrayList<HumanCard> registers, ArrayList<HumanCard> cards, Bot robi) {
        ...
        // in vonPermut steht die vorgeschlagene Registerprogrammierung, so wie
        // sie ggf. z.T.schon
        // in den Registern steht
        int nextprio = -1; // will contain the card (identified by its unique
                           // priority) we recommend
        for (int registerNum = 0; registerNum < 5; registerNum++) {
            // looking for the register we have to program:
            HumanCard card = registers.get(registerNum);
            if (card == null || (card.getprio() != vonPermut[registerNum].getprio())) {
                // found an empty or suboptimal programmed register
                nextprio = vonPermut[registerNum].getprio(); // setting our
                                                             // recommendation
                lastPredict = registerNum; // updating the marker for the last
                                           // predicted register
                break;
            }
        }
    ...
    protected Card[] getPredictionCards(ArrayList<HumanCard> registers, ArrayList<HumanCard> cards, Bot robi) {
        // check if all register slots have a card
        int t = 0;
        for (t = 0; t < 5; t++) {
        ...
        for (int i = 0; i < cards.size(); i++) {
            if ((cards.get(i) != null) && cards.get(i).getState() == HumanCard.FREE) {
                simCards[j] = new HumanCard(cards.get(i));
                j++;
            }
        }
        CAT.debug("simCards:\n" + Global.arrayToString(simCards));
        // gelegte Karten in das enstprechende gesperrte Register des Robis
        // packen
        for (int l = 0; l < registers.size(); l++) {
            if ((registers.get(l) != null) && !(registers.get(l).free())) {
                simRob.lockRegister(l, Deck.get(registers.get(l).getprio()));
            }
        }
        CAT.debug("SIMROB1:\n " + simRob);
    ...
    /**
     * Will find the index of the card with priority <code>nextprio</code> in
     * the HumanCard list <code>robicards</code>. If the card is not found it
     * will return 0 (==the first card; don't know whether that is such a good
     * idea but, hey, I didn't write the code and it seems to work this way..).
     * 
     * @param nextprio
     *            The unique priority of the card to look for
     * @param robiCards
     *            The possible cards (of <code>HumanCards</code>)
     * @return the list position of the card we are looking for in the list
     */
    public static int getIndex(int nextprio, ArrayList<HumanCard> robiCards) {
        CAT.debug("Wisenheimer.getIndex() called");
        int ind = 0;
        for (int i = 0; i < robiCards.size(); i++) {
            if (robiCards.get(i) != null) {
                if (robiCards.get(i).getprio() == nextprio) {
                    ind = i;
                    break;
                }
            }
    ...
    public int getNextPrediction(ArrayList<HumanCard> registers, ArrayList<HumanCard> robiCards) {
