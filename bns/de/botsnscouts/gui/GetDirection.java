...
public class GetDirection extends TJPanel {
    ...
    private static Color arrowColor = GreenTheme.getTextColor();// new
                                                                // Color(64,191,64);
    ...
    public GetDirection(ActionListener al) {
        ...
        PfeilC no = new PfeilC(Directions.NORTH);
        no.addActionListener(al);
        p.add(no);
        p.add(Box.createGlue());

        PfeilC we = new PfeilC(Directions.WEST);
        we.addActionListener(al);
        p.add(we);
        // p.add(new JButton());
        p.add(Box.createGlue());
        PfeilC os = new PfeilC(Directions.EAST);
        os.addActionListener(al);
        p.add(os);
        p.add(Box.createGlue());
        PfeilC su = new PfeilC(Directions.SOUTH);
    ...
    private class PfeilC extends JButton {
        ...
        public void paintComponent(Graphics g) {
            ...
            if (richt == Directions.NORTH) {
                ...
                if (richt == Directions.SOUTH) {
                    int[] x = { 5, 55, 30 };
                    int[] y = { 5, 5, 55 };
                    g.fillPolygon(x, y, 3);
                }
                else
                    if (richt == Directions.WEST) {
                        int[] x = { 55, 55, 5 };
                        int[] y = { 5, 55, 30 };
                        g.fillPolygon(x, y, 3);
                    }
