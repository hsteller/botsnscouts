...
public class WinnerListPanel extends JPanel {
     ...
     * 
     * @param names
     *            The robot's names as given by the AusgabeView
     * @return the values given in the parameter array (but without possible
     *         NULL-entries)
     */
    ...
    private void init() {
        ...
        for (int row = 0; row < rowCount; row++) { // lines for robots
            for (int col = 0; col < COL_COUNT; col++) { // colum values for
                                                        // robot l
                if (col == NAME_COL) { // colum with names
                    ...
                    this.add(desc[row][col], new GridBagConstraints(col, row + 1, 1, 1, 0.0, 0.0,
                                    GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(2, 10, 10, 0),
                                    0, 0));
                }
                else {
                    desc[row][col] = new TJLabel("", SwingConstants.LEFT);
                    this.add(desc[row][col], new GridBagConstraints(col, row + 1, 1, 1, 0.0, 0.0,
                                    GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(2, 10, 10, 0),
                                    0, 0));
                }
            }
        }
        ...
        TitledBorder tb = BorderFactory.createTitledBorder(redline, "RANKING", TitledBorder.CENTER, TitledBorder.TOP,
                        new Font("Sans", Font.BOLD, 30), Color.YELLOW.darker());
    ...
    public static void main(String args[]) {
        ...
        HashMap<String, Integer> ht = new HashMap<String, Integer>(robs.length);
