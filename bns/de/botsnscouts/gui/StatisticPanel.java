...
public class StatisticPanel extends ColoredPanel implements ActionListener {
    ...
    private static final TJLabel[] COL_HEADINGS = {
            ...
            new TJLabel(Message.say(MESSAGE_SECTION, "name"), SwingConstants.LEFT),// robot
                                                                                   // name
    ...
    private void init() {
        ...
        /*
         * GridLayout grid = new GridLayout(l+1, COL_COUNT); grid.setHgap(5);
         * grid.setVgap(5);
         */
        ...
        this.add(COL_HEADINGS[0], new GridBagConstraints(0, 0, 2, 1, 0.0, 0.0, GridBagConstraints.CENTER,
        ...
        for (int i = 1; i < COL_COUNT - 1; i++) {
            ...
            this.add(COL_HEADINGS[i], new GridBagConstraints(i + 1, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
        ...
        for (int i = 0; i < l; i++) { // lines for robots
            for (int j = 0; j < COL_COUNT; j++) { // colum values for robot l
                if (j == 1) { // colum with names should be aligned left
                    ...
                    this.add(desc[i][j], new GridBagConstraints(j, i + 1, 1, 1, 0.0, 0.0, GridBagConstraints.WEST,
                                    GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
                }
                else {
                    desc[i][j] = new JLabel("", SwingConstants.CENTER);
                    this.add(desc[i][j], new GridBagConstraints(j, i + 1, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
                                    GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
                }
            }
        }
