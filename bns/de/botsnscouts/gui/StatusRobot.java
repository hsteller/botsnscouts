...
public class StatusRobot extends JButton {
    ...
    // The following three objects are needed for the "advanced tooltip" this
    // class has:
    // it should not only show a text ("click robot to..") but also show the
    // robots registers
    // (including the current cards)
    private PaintPanel registerPanel = new PaintPanel(OptionPane.getBackgroundPaint(this), true);

    // private ColoredPanel registerPanel= new
    ...
    public StatusRobot() {
        this(new ImageIcon(RobotInfo.roboImages[0]), new ImageIcon(RobotInfo.cursors[0]));
    }
    ...
    public void paintComponent(Graphics _g) {
        ...
        if (lifesLeft == 2)
            smallBot.paintIcon(this, g, w, 6);
        if (lifesLeft >= 1)
            smallBot.paintIcon(this, g, w, smallBot.getIconHeight() - 3);
        if (robot != null && !robot.isActivated()) {
            g.setComposite(comp);
        }
    }
    ...
    public void setRoboRegisters(ScalableRegisterRow robsRegisters) {
        if (robsRegisters == null) {
            ...
            if (this.registers == null) {
                ...
                // registerPanelBg.add(registerPanel); // hack to get a darker
                // background
