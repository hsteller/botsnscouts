 ...
 * 
 * 
 *          More or less a hack. Is intended to keep track of the server and clients/BNSThreads that were launched on localhost; to make sure that the clients get killed if the server isn't running anymore. General purpose for that: being able to do
 *          a "clean" quit of local HumanPlayers/games (==redisplay the main launcher app to maybe start a new game instead of the former "System.exit(0)" or the current "just hang up")
 * 
 */
public class Registry implements ShutdownListener, GameOverListener, GameStateListener {
    ...
    /**
     * If set to "true", we will attempt to redisplay the main menu (leaks lotsa memory); => if set to false, we will almost always kill the JVM and try to restart the game in a new one, EXCEPT: if "aGameHasStarted" is still false, we will try a
     * redisplay without killing the JVM, as I think the memory trouble might not be so severe if the game has not started yet (=> the clients didn't do much, only registered at the server and the views didn't load the graphics for the gamebaord
     * yet..)
     * 
     */
    ...
    private HashMap<Shutdownable, Game> shutdownablesToGames = new HashMap<Shutdownable, Game>();
    ...
    /**
     * Must be called to activate the registry. Should be called when the program is started the 'normal' way (== program started with the main menu GUI, not a single player started via CLI in its own JVM; in the CLI-single-player-case we shouldn't
     * enable the Registry to avoid an automatic launch of the main menu when the CLI-player shuts down).
     */
    ...
    /**
     * Tries to find a game with the given parameters The serverIp and the corresponding port are combined, can identify a game on a server
     * 
     * @param serverIp
     *            the IP
     * @param port
    ...
    private Game findGame(Shutdownable clientOrServer) {
        Game g = shutdownablesToGames.get(clientOrServer);
    ...
    protected void addClient(String serverIp, int port, Shutdownable client, int clientInfoClientType) {
        ...
        CAT.debug("addClient: serverIp=" + serverIp + ";port=" + port + ";type=" + clientInfoClientType + "; client="
                        + client);
        Game game = findGame(serverIp, port);
        if (game == null) {
            CAT.info("Registry: Tried to add a client to a non-existing game on " + serverIp + ":" + port
                            + "; this is ok if you participate in a non-local game");
    ...
    public String dump() {
        ...
        synchronized (shutdownablesToGames) {
            sb.append("\n\nshutdownables->games:\n");
            for (Iterator<Shutdownable> keys = shutdownablesToGames.keySet().iterator(); keys.hasNext();) {
    ...
    /*
     * public boolean removeClient(Shutdownable client){ Game game = findGame(client); boolean success = false; if (game != null){ success = game.removeClient(client); } else {
     * CAT.error("Registry: Tried to remove a client from a non-existing game!"); } clientsToGames.remove(client); return success; }
     */
            ...
            for (Game g : games) {
                if (g.hasHumanView()) {
                    ...
                    if (server != null) {
                        ...
                        CAT.error("This is strange; there is a serverless game registered but nobody "
    ...
    /**
     * Note: This method is supposed to be called by Shutdownables in their shutdown() method. It will NOT shut down <code>someThread</code> or we might get an infinite loop. It is supposed to notify the registry that someThread has killed off its
     * communications. HOWEVER: This method WILL KILL other threads/players/views/servers and redisplay the main menu if <code>someThread</code> was the last view of a (local) game..
     */

    @SuppressWarnings("unused")
    // the block at the bottom is unused depending on the value of the constant:
    // if (!RESTARTS_BY_KILLING_JVM && pseudoBoolean[0] == null){
    public void shutdown(Shutdownable someThread) {
        ...
        if (someThread instanceof Server) {
            ...
            CAT.debug("remove of Thread " + someThread + (removeSuccessful ? "" : "NOT ") + "successful");
            if (removeSuccessful) {
     ...
     */
    /*
     * private static String maskWhiteSpace(String s){ if (s == null){ return null; } int length = s.length(); StringBuffer sb = new StringBuffer(length+10); char space = ' '; for (int i=0;i<length;i++){ char c = s.charAt(i); if (c == ' '){
     * sb.append('\\'); } sb.append(c); } if (length!=sb.length()){ return sb.toString(); } else { return s; } }
     */
    /**
     * This one is a heavy hack Kills the VM an starts a new one
     * 
     */
    ...
    private void registryRemoveAndKillAutobots(Game game) {
        Collection<ClientInfo> bots = game.removeAndKillAllAutoBots();
        if (bots != null) {
            synchronized (bots) {
                for (ClientInfo ci : bots) {
                    synchronized (shutdownablesToGames) {
...
class Game {
    ...
    private LinkedList<GameOverListener> gameOverListeners = new LinkedList<GameOverListener>();
    ...
    /**
     * Somewhat ugly that 'addClient()' has a 'ClientInfo' as parameter and 'removeClient()' a Shutdownable. Reason: 'addClient()' is supposed to be called by the launcher application that knows what client it starts (and we want that to know that
     * information here). But most (all?) calls of 'removeClient' will happen in BNSThread (closer: its 'ShutdownableSupport') and without using lots of 'instanceof' or a client-type field in BNSThread we don't know how to create a proper
     * ClientInfo-object to pass to removeClient. And even IF we create it, we still need to modify ClientInfo.equals() so it is somewhat simpler to modify ClientInfo.equals() in a way that it works with CllientInfos AND Shutdownables.
     * 
     ...
     * Somewhat ugly that 'addClient()' has a 'ClientInfo' as parameter and 'removeClient()' a Shutdownable. Reason: 'addClient()' is supposed to be called by the launcher application that knows what client it starts (and we want that to know that
     * information here). But most (all?) calls of 'removeClient' will happen in BNSThread (closer: its 'ShutdownableSupport') and without using lots of 'instanceof' or a client-type field in BNSThread we don't know how to create a proper
     * ClientInfo-object to pass to removeClient. And even IF we create it, we still need to modify ClientInfo.equals() so it is somewhat simpler to modify ClientInfo.equals() in a way that it works with ClientInfos AND Shutdownables.
     * 
     * ..or this is simply crazy 3:00 AM thinking ;-)
     * 
     * 
    ...
    public boolean removeClient(Shutdownable client) {
        ...
        synchronized (clients) {
            if (clients != null) {
                ...
                CAT.debug("removing of 'Shutdownable' from the 'ClientInfo' collection was " + (success ? "" : "NOT ")
                                + "successful");
    ...
    /**
     * Removes all clients of type <code>type</code> that are associated with this game and returns them
     * 
     * @param type
     *            a client type as specified by the constants in this class
     * @return All ClientInfos of type <code>type</code> that were removed (currently in a LinkedList)
     */
    private Collection<ClientInfo> removeClients(int type) {
        LinkedList<ClientInfo> remClients = new LinkedList<ClientInfo>();
        synchronized (clients) {
    ...
    protected Collection<ClientInfo> removeAndKillAllAutoBots() {
        CAT.debug("removeAndKillAutoBots");
        Collection<ClientInfo> autoBots = this.removeClients(Game.CLIENT_TYPE_AUTOBOT);
        int numOfBots = autoBots != null ? autoBots.size() : 0;
        CAT.debug("num of bots: " + numOfBots);
        for (ClientInfo bot : autoBots) {
...
/**
 * Somehow not in the mood for "instanceof" right now, so here comes another inner class.. ;-) And who knows, there might be other info besides the client type that we want to keep later.
 * */
class ClientInfo {
     ...
     * 
     * @param client
     *            Uuuh..the client we want to store some additional info for.
     * @param clientType
     *            whether this client is an AutoBot, HumanPlayer, View,..
     */
    ...
    /**
     * Can be used to compare this ClientInfo object to another ClientInfo _OR_ to compare it with an instanceof 'Shutdownable'; in the second case this ClientInfo's internal Shutdownable is compared (using '==') with the Shutdownable that was passed
     * as parameter
     * 
     * @param o
     *            another Object to compare this Clientinfo to; may also return true in case of o being a Shutdownable, not a ClientInfo
    ...
    /*
     * public int hashCode(){ CAT.debug ("hashCode called"); Shutdownable mine = this.getClient(); if (mine != null){ return mine.hashCode(); } else { return super.hashCode(); }
     * 
     * }
     */
    public String toString() {
        ...
        return "client class=" + client.getClass() + "; name=" + (name == null ? "N/A" : name) + "; clientType="
                        + clientType;
    }
...
/**
 * Whats the difference to the GameStateListener? void gameIsOver(Game game);
 */
// hendrik says:
