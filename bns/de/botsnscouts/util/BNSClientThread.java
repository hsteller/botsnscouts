    ...
    public void bnsStart() throws JoiningGameFailedException{
        ...
        try {
           ...
           synchronized(myWindowsSucksVolatileHack){
               ...
               myWindowsSucksVolatileHack.wait(1);
            	// also, we MUST USE WAIT AND JOIN
            	// removing the join() and using wait(MAX_RETRIES*..) also had
            	// the above effect
            	// NOTE: don't use wait() because in the more common case that there
                // is no registration error, the Thread "t" will have called "notify()" before 
               //  wait() is called
           }
        }
