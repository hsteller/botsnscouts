...
public class CursorMan {
    public final static int CURSOR = 0, BOTCENTER = 1, STATUSROBOTS = 2, DAMAGE = 3, WINNER = 4;

    static final Category CAT = Category.getInstance(ImageMan.class);

    final static ImageSet[] imgSetDescr = { ImageSet.getInstance("images/cursor.gif", 5, 5),
            ImageSet.getInstance("images/botcenter.gif", 5, 5), ImageSet.getInstance("images/minibots.gif", 8, 1),
            ImageSet.getInstance("images/damage.gif", 1, 1), ImageSet.getInstance("images/winner.gif", 8, 8) };
    ...
    synchronized static void loadImages() {
        ...
        for (int i = 0; i < IMGSETCOUNT; i++) {
            ...
            Image img = tk.getImage( de.botsnscouts.BotsNScouts.class.getResource(descr.name));
        ...
        imageLoader = new BNSThread("CursorMan") {
            public void doShutdown() {
                CursorMan.CAT.debug("CursorMan's empty doShutdown() called");
            }
