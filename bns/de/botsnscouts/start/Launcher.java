...
public class Launcher implements RegistrationStartListener {
    ...
    /**
     * Only created so this class and the static methods can implement and use the RegistrationListener interface. ParticipateInAGame will use this
     * method to synchronize on so that the player won't be launched until the server has signaled that the
    ...
    public static BNSThread startAutoBot(String ip, int port, int iq, boolean beltAware, String botName) {
    ...
    public Server startGame(GameOptions options, ServerObserver listener) {
