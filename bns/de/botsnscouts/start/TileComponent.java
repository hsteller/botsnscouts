...
public class TileComponent extends JComponent implements MouseListener, MouseMotionListener {
    ...
    public void rasterChanged() {
        ...
        for (int i = 0; i < flag.length; i++) {
            if (flag[i] != null && (flag[i].x - 1) / 12 == myX && (flag[i].y - 1) / 12 == myY) {
                flagsChanged = (flagsChanged || flagNum.length <= counter || flagNum[counter] != i + 1
                                || flagPos[counter].x != (flag[i].x - 1) % 12 || flagPos[counter].y != (flag[i].y - 1) % 12);
                counter++;
            }
        }
    ...
    public void paint(Graphics g) {
        ...
        if (tileImage == null) {// falls keine Kachel an dieser Stelle
            ...
            g2d.drawImage(tileImage, AffineTransform.getRotateInstance(Math.toRadians(-90 * drehung),
                            tileImage.getWidth(null) / 2, tileImage.getHeight(null) / 2), null);
            ...
            for (int i = 0; i < flagNum.length; i++) {
                ...
                g2d.drawString("" + flagNum[i], (int) ((double) flagPos[i].x * (double) W / 12. + 2), H
                                - (int) ((double) (flagPos[i].y) * (double) H / 12.) - 2);
            }
        }
    }
    ...
    public void mouseClicked(MouseEvent e) {
        if (tileClickListener != null) {
            ...
            // unnecessary casts above, better and faster:
            int xx = (e.getX() == W ? 12 : (int) (e.getX() * 12.0 / W) + 1);
            // y->(H-y)*12/H +1
            // int yy = (e.getY()==0?12:(int)((double)(H-e.getY())*12.0/(double)H)+1);
            // unnecessary casts above, better and faster:
            int yy = (e.getY() == 0 ? 12 : (int) ((H - e.getY()) * 12.0 / H) + 1);
