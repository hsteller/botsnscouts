...
/**
 * This class contains the communication stuff that is only relevant for a
 * view("Ausgabe")
 * 
...
public class KommClientAusgabe extends KommClient {
     ...
     * @param port
     *            the port the server uses to listen for clients to register
     * @param name
     *            the name of this view
     * @return true if the registration was successful
     * 
     * @exception KommException
     *                thrown if parsing of the server answer failed or in case
     *                of an I/O error
     * @exception RegistrationException
     *                thrown if the server has a logical reason to deny
     *                registration (too many players, name already in use..)
     **/
    ...
    // here: name encoding not really needed, because we use our generated
    // random
    ...
    /**
     * The "answer" for a "notify change"-message from the server. Sends a
     * confirmation to signal he server that we have done (displayed) all the
     * stuff we wanted and the server may continue (with the phase evaluation).
     */
    ...
    public StatsList getStats() throws KommException {
        ...
        if (rein.startsWith("GST")) {
            Vector<Stats> stats = new Vector<Stats>(8);
