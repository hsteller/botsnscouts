 ...
 * sendRobStatus /** Oberklasse f�r SERVER-Kommunikation <BR> Enth�lt Objekte
 * und Methoden, die sowohl zur Kommunikation mit Spielern als auch zur
 * Kommunikation mit Ausgabekan�len benutzt werden k�nnen @author Hendrik <BR>
 *  
 */
public class KommServer {
    ...
    /**
     * Diese Methode ist eigentlich nicht da; sie ist nur zu Testzwecken public,
    ...
    ServerAntwort wait2(String input) throws KommException {
        ...
        try {
             ...
             * einem String verkehrt geklammert wurde (z.B. ")xyz(" statt
             * "(xyz)"); leider zu spaet aufgefallen, um exakte Fehlerstelle
             * (bearbeitete Kommunikationsart) ohne grossen Aufwand ausgeben zu
             * koennen. Dazu waeren noch viel mehr try/catch-Statements noetig,
             * da diese Exceptions bei jedem Aufruf von "substring" oder
             * "charAt" auftreten koennen.
             */
            try {
                ...
                // Exception kann wegen min. Laenge der Strings (=2 Zeichen)
                // nicht auftreten
                error = true;
                errormsg = "Weniger als zwei Zeichen im String";
                // zwei dummys, damit der Compiler nicht behauptet, st und nd
                // w�rden (unter Umstaenden) nicht initialisiert werden
                st = '~';
                nd = '~';
            }
            /*
             * abfangen des einzigen Falls, f�r den der String nicht mindestens
             * drei Zeichen lang ist
             */
            if (st == 'O') {
                if (nd == 'K') {
                    back.typ = ServerAntwort.AENDERUNGFERTIG;
                    back.ok = true;
                }
                else {
                    error = true;
                ...
                try {
                    ...
                    // exception d�rfte nicht auftreten..
                    rd = '~'; // wg. Compiler (s.o.)
           ...
           * try wegen IndexOutOfBoundsExceptions; leider zu spaet aufgefallen,
           * um exakte Fehlerstelle (bearbeitete Kommunikationsart) ausgeben zu
           * koennen.
           */
    ...
    /**
     * Dient dazu, einem Client mitzuteilen, da� er vom Spiel ausgeschlossen
     * wurde. Bekommt den Grund als String �bergeben, am besten schon in der
    ...
    /**
     * Zur Antwort auf Info-Request 'gibFahnenPos'. Erh�lt die Fahnenpositionen
    ...
    /**
     * Zur Antwort auf Info-Request 'gibNamen'. Erh�lt ein String-Array mit den
     ...
     * zustaendig ist. Das Array enthaelt die Namen der Spieler an speziellen
     * Positionen. Alle anderen Elemente sind null.
     */
    ...
    /**
     * Zur Antwort auf Info-Request 'gibRoboterPos'. Erh�lt den Location des
    ...
    /**
     * Zur Antwort auf Info-Request 'gibRobStatus'. Erh�lt ein Bot-Objekt mit
    ...
    /**
     * F�r die Protokollerweiterung: Wurde der Status eines entfernten Roboters
    ...
    /**
     * Zur Antwort auf die Abfrage des Timeouts.Erh�lt als Argument die Dauer
    ...
    /**
     * Zur Antwort auf Info-Request 'gibAuswertungsstatus'. Erh�lt f�r jeden Bot
    ...
    public void spielStatus(Status[] robbis) throws KommException {
        ...
        try {
            // debug-part
            /*
             * System.err.println("SERVER: Status: "); for (int i=0;i
             * <robbis.length;i++) System.err.println(robbis[i].toString());
             */
            // d-p ende
    ...
    /*
     * Zur Benachrichtigung der Spieler, dass das Spiel anf�ngt. (true=ok,
     ...
     * ((a.equals("ok"))||(a.equals("OK"))) return true; else if
    ...
    /**
     * Zur Benachrichtigung der Spieler, dass das Spiel anf�ngt. Ob ein ok
     * zur�ckkkommt, kann mithilfe der Warte-Methode bestimmt werden (Fall
