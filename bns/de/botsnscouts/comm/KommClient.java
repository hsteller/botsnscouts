...
/**
 * The parent class for all clientside communication. Very 'advanced' parsing of
 * Strings.
 * 
 * @author Hendrik
 */
...
public class KommClient implements Shutdownable {
    ...
    /**
     * Needed for a maybe deprecated hotfix. This is a flag that is set if the
     * server sent an 'NTC' (notify change) at an unexpected time (while waiting
     * for a request's response).
     */
    protected boolean gotNTC;

    /**
     * Needed for a maybe deprecated hotfix. Saves the NTC-String sent at a
     * wrong time. Together with 'gotNTC' it represensts a kind of an
     * one-element-queue ;-)
     */
    protected String strNTC;

    /**
     * Used for communication with the server. The (URL-)encoded name of the
     * client; will be used in methods instead of names that are given as
     * parameters.
     * 
     */
    ...
    /**
     * The name of the client that owns this KommClient object. For debugging
     * purposes.
     */
    protected String cn;

    /**
     * For debugging purposes: if set to true, a logfile
     * "<clientname>'s.Kommlog" will be created, saving all Strings the client
     * sent or got. Can be changed in source code only => For
     * activating/deactivating you'll have to recompile this class.
     */
    protected final boolean log = false;

    /**
     * To implement the Shutdownable interface; will be set to true on the end
     * of the shutdown() method
     */
    ...
    private TmpParsedSeqNumMessage splitPossibleSeqNumAndMessage(String com) throws KommException {
        ...
        if (com.startsWith(OtherConstants.MESSAGE_NUMBER)) {
            ...
            String seqString = com.substring(0, kommaPos); // contains the a
                                                           // String like
                                                           // "message_number=1234"
            ...
            back.messageNum = -1; // important to indicate that there was no
                                  // sequence number
    ...
    /**
     * This method reads messages sent by the server, if a special request has
     * been sent (i.e. used in all non-void get-Methods). It is to be used
     * instead of 'warte' (or 'wait2'). Unexpected sent 'REN' or 'NTC' will be
     * treated here. If 'REN' (Client removed by server) was read, a
     * KommFutschException will be thrown; if 'NTC' was read, gotNTC will be set
     * to true, the NTC-String will be saved and the next incoming String will
     * be returned.
     */
    @SuppressWarnings("unused")
    // for if(log) blocks if log is set to "false"
    protected String einlesen() throws KommException {
        ...
        if (back == null) {
            try {// if null, try again
                ...
                if (back == null)// read null the second time => no connection
                                 // anymore => exception
                    throw new KommFutschException("Einlesen: read 'null' two times");

            }
            catch (IOException ioe2) {
                throw new KommFutschException("Einlesen: IOException while retrying to read; Message: "
                                + ioe2.getMessage());
            }
        }
        // Tried two times to read the String

        // removing possible sequence number part from back for testing for
        // REN/NTC
        ...
        if ((test.length() >= 3) && (test.substring(0, 3).equals("REN"))) {
            ...
            try {
                ...
                throw new KommException("einlesen: REN erhalten, aber der String \"" + back
                                + "\" verursachte in wait2 eine Exception;\n Message:" + e.getMessage());
            }
        ...
        // Now checking whether we got a bad timed NTC (notify change)
            ...
            if ((test.length() >= 3) && (test.substring(0, 3).equals("NTC"))) {

                gotNTC = true; // changing state; indicating that there is an
                               // NTC waiting
    ...
    /**
     * The main method of this class. It waits for messages sent from the
     * server.
     * 
     * @exception KommException
     *                Any error parsing a server-message (including most
    ...
    ClientAntwort wait2(String com) throws KommException {
        ...
        back.messageSequenceNumber = foo.messageNum; // will be set to -1 if
                                                     // there
        ...
        if ((com.equals("ok")) || (com.equals("OK"))) {
            ...
            if ((com.equals("error")) || (com.equals("ERROR"))) {
                back.typ = ClientAntwort.ANGEMELDET;
                back.ok = false;
            }
            else {
                        ...
                        case 'M': {
                            if ((nd == 'S') && (rd == 'G')) { // some generic
                                ...
                                if (seq.startsWith(OtherConstants.MESSAGE_NUMBER)) {
                                    ...
                                    // no sequenznummber, use everything
                                    back.namen = tmp;
                                if (CAT.isDebugEnabled()) {
                                ...
                                if (nd == 'R') {
                                    if (rd == 'P') { // got cards
                                        try {
                                            try {
                                                Card[] karten1 = new Card[9]; // max.
                                                                              // number
                                                                              // of
                                                                              // cards
                                                int klammerauf = com.indexOf('(');
                                                // int
                                                // klammerletzt=com.lastIndexOf(')');
                                                ...
                                                while (com.length() > 1) {
                                                    ...
                                                    String work = com.substring(klauf + 1, klzu); // first
                                                                                                  // card,
                                                                                                  // without
                                                                                                  // parens,
                                                                                                  // i.e.
                                        ...
                                        if (rd == 'R') { // may repair a locked
                                            ...
                                            try {
                                                ...
                                                try {
                                                    ...
                                                    error = true;
                                                    throw new KommException(
                                                                    "MRR -> Fehler beim Parsen der Registerzahl");
                                                }
                                            }
                                            catch (Exception e) {
                                                error = true;
                        ...
                        case 'N': {
                            if (nd == 'T') {
                                if (rd == 'S') { // game starts
                                    ...
                                    if (rd == 'C') { // something has changed
                                                     // for one or more robots
                                        incNTCCounter();
                                        try {
                                            back.typ = ClientAntwort.AENDERUNG;
                                            int klauf = com.indexOf('(');
                                            // getNamen2 does the URL-decoding
                                            // of the names for us..
                            ...
                            if (nd == 'S') { // current standings
                                try {
                                    ...
                                    if (com.length() == 7) {
                                        ...
                                        com = "(" + com; // now the String has
                                                         // GSN-formatting
                                        // getNamen2 does the URL-decoding of
                                        // the names for us..
                                ...
                                if (nd == 'A') { // game status, which phase are
                                                 // we in
                                    try {
                                        back.typ = ClientAntwort.SPIELSTATUS;
                                        if (com.length() <= 4)
                                            back.ok = false; // => no phase
                                                             // processing
                                        else {
                                            back.ok = true; // => phase
                                                            // processing
                                            int klauf = com.indexOf('('); // no
                                                                          // we
                                                                          // are
                                                                          // behind
                                                                          // 'SA'
                                            int phase = 0; // init
                                            try {
                                                phase = Integer.parseInt(com.substring(klauf + 1, klauf + 2));// parsing
                                                                                                              // phase-number
                                            }
                                            catch (NumberFormatException nfe) {
                                                throw new KommException(
                                                                "NumberFormatException bei getSpielstatus (Parsen der Registerphase); Message: "
                                                                                + nfe.getMessage());
                                            }
                                            int komma = com.indexOf(',');
                                            com = com.substring(komma + 1); // now
                                                                            // there
                                                                            // are
                                                                            // only
                                                                            // names
                                                                            // left
                                            Status[] stats = new Status[8]; // 8=maximum
                                                                            // number
                                                                            // of
                                                                            // players
                                    ...
                                    if (work.length() <= 2) {
                                        if (work.equals("LL")) {// lost all
                                            ...
                                            if (work.equals("TO")) {
                                                ...
                                                if (work.equals("GO")) {
                                                    back.str = Message.say("comm", "removalReasonGO"); // game
                                                                                                       // is
                                                                                                       // over
                                                    ...
                                                    if (work.equals("RV")) {
                                                        back.str = Message.say("comm", "removalReasonRV"); // rule
                                                                                                           // violation
                                                        back.zahl = ClientAntwort.REMOVAL_REASON_RULEVIOLATION;
                                                    }
                                                    else
                                                        if (work.equals("ZS")) {
                                                            back.str = Message.say("comm", "removalReasonZS"); // to
                                                                                                               // late
                                                                                                               // registered
                                                            back.zahl = ClientAntwort.REMOVAL_REASON_TOOLATE;
                                                        }
                                                        else {
                                                            back.str = Message.say("comm", "removalReasonOther", work); // unknown
                                                                                                                        // reason
     ...
     * 
     * @param name
     *            The name of the client UPDATE: WILL BE IGNORED lol
     */
    ...
    /**
     * This method registers a client. Never used directly. Use child's method
     * instead. DOES NOT ENCODE THE CLIENT NAME; ENCODING MUST HAPPEN IN THE
     * DERIVED CLASSES ("anmelden*"-methods)
     * 
     * @param ipnr
     *            IP of the server
     * @param portnr
     ...
     * @param kuerzel
     *            Indicates, whether the client is a robot or an output channel
     * @exception KommException
    ...
    protected boolean anmelden(String ipnr, int portnr, String clientName, String kuerzel) throws KommException,
        ...
        encodedName = st.nextToken();// clientName; // now encoded in derived
                                     // classes, only to lazy
        ...
        try {
            ...
            if (answerLC.equals("ok")) {
                ...
                if (antwort.toLowerCase().startsWith("error")) {
            ...
            throw new KommException("Error registering at the server: IOException during read: " + ie.getMessage());
        }
     ...
     * 
     * @return An array of 'Orts' (equal to Java's 'Point'). They are ordered
     *         according to the flags' numbers.
    ...
    public Location[] getFahnenPos() throws KommException {
        ...
        int zaehler = 0;
        int start = 0;
        while (pos != -1) { // in dieser Schleife sollte keine Exception
                            // auftreten
        ...
        try {
            ...
            for (int myI = 0; myI < zaehler; myI++) {
                ...
                String x = rein.substring(1, kommapos); // String containing
                                                        // x-coordinate
                String y = rein.substring(kommapos + 1, klammerzu); // String
                                                                    // containing
                                                                    // y-coordinate
                int xk = Integer.parseInt(x); // x-coordinate
                int yk = Integer.parseInt(y); // y-coordinate
                back[myI] = new Location(xk, yk);
                rein = rein.substring(klammerzu + 1);// remove parsed Point
                                                     // (Location)
            }
        }
        // A lot of errors might happen.. :
        catch (StringIndexOutOfBoundsException sioob) {
            throw new KommException("getFahnenPos warf StringIndexOutOfBoundsException(Inhalt:" + sioob.getMessage()
                            + "); Ursache: vermutlich falsch aufgebaute Antwort vom Server");
        }
        catch (NumberFormatException nfe) {
            throw new KommException(
                            "getFahnenPos: NumberFormatException: Parsen der koordinaten-substrings schlug fehl");
        }
        catch (ArrayIndexOutOfBoundsException aioob) {
            throw new KommException("getFahnenPos warf ArrayIndexOutOfBoundsException (Inhalt: " + aioob.getMessage()
                            + "); der Fehler liegt wahrscheinlich an KommClient)");
        }
        catch (Exception sonstige) {
            throw new KommException("getFahnenPos warf beim Parsen eine unerwartete Exception; Message: "
                            + sonstige.getMessage());
        }
        return back;
    }
     ...
     */
    /*
     * public Location getRobPos (String name) throws KommException { return
     * this.fetchOrt(name, true); }
     */
    /**
    ...
    public String getSpielfeld() throws KommException {
        ...
        try {
            ...
            back = rein;
            if (rein.lastIndexOf('.') == -1) // the end of the boards
                                             // description reached (ending with
                                             // a dot)?!
                back += "\n"; // no => new line
            while (rein.lastIndexOf('.') == -1) {
                rein = this.einlesen();
                back += rein; // concat parts
                if (rein.lastIndexOf('.') == -1) // make sure that no line break
                                                 // is following the dot
                    back += "\n";
            }
    ...
    /**
     * Supporting method for parsing a list of names.. .. or other Strings
     * formatted the same way: "(String1,String2,..,StringN)"
     */
    ...
    /**
     * Zur Frage nach den Farben der Spieler. Ein Array-Element ist entweder
     * null oder einer der Namen.
     */
    public String[] getFarben() throws KommException {
        ...
        /*
         * for (int i=0;i<back.length;i++) { if (back[i]=="0") back[i]=null; }
         */
        return back;
    ...
    /**
     * Info-Request zur Abfrage des Status eines Roboters; gibt ein Bot-objekt
     * zur&uuml;ck, dass alle notwendigen Informationen enth&auml;lt. Falls der
     * Bot entfernt wurde, wird Bot.leben auf -1 gesetzt, die restlichen
     * Attribute werden nicht gesetzt (d.h = null oder was_auch_immer).
     * 
     * @exception KommException
    ...
    public Bot getRobStatus(String name) throws KommException {
        ...
        this.senden(raus);
        // Server sends
        // "RS(Richtung(N,O..), Location(1,1), LFlag, LArchF, Schaden, VLeben, GespRegister, Aktiv, Virtuell, RSreserveiert)"
        /*
         * try{ com=in.readLine(); // System.out.println
         * ("GETROBSTATUS: gelesener String: "+com); } catch(IOException ioe){
        ...
        if (com.equals("RSE")) {
            ...
            try {
                ...
                throw new KommException("getRobStatus: (Location-Parsen) NumberFormatException(Message: "
                                + nfe.getMessage());
            }
            ...
            try {
                robot.setNextFlag(Integer.parseInt(flagge) + 1);// +1, weil
                                                                // unser Bot die
                                                                // naechste und
                                                                // nicht die
                                                                // letzte Flagge
                ...
                throw new KommException("getRobStatus: (L-Flag-Parsen) NumberFormatException(Message: "
                                + nfe2.getMessage());
            }
            ...
            try {
                ...
                throw new KommException("getRobStatus: NumberFormatException (Archiv-Parsen)(Message: "
                                + nfe3.getMessage());
            }
            ...
            try {
                ...
                throw new KommException("getRobStatus: NumberFormatException (Schaden-Parsen)(Message: "
                                + nfe4.getMessage());
            }
            ...
            try {
                ...
                throw new KommException("getRobStatus: NumberFormatException (Leben-Parsen)(Message: "
                                + nfe5.getMessage());
            }
            com = com.substring(kommapos + 1, com.length() - 1);// vLeben und
                                                                // Komma
                                                                // entfernen;
                                                                // sowie die
                                                                // abschliessende
            ...
            klazupos = com.lastIndexOf(')'); // das muesste jetzt die letzte
                                             // Klammer des Karten-Teilstrings
                                             // sein
            com = com.substring(klazupos + 1); // Karten-Teilstring entfernen

            // com sieht jetzt entweder "<Bool>,<Bool>" oder ",<Bool>,<Bool>"
            // aus
            if (com.charAt(0) == ',')
                com = com.substring(1); // eventuelles Komma vor erstem Boolean
                                        // entfernen
    ...
    /**
     * Info-Request zur Abfrage des Spielstandes. Falls das Spiel beendet wurde,
     * enth&auml;lt das String-Array die Namen der Spieler, wobei der Name des
     * Gewinners an erster Stelle steht. L&auml;uft das Spiel noch, wird null
     * zur&uuml;ckgegeben.
     * 
     * @exception KommException
     *                Tritt beim Parsen ein Fehler auf (z.B. wegen falsch
    ...
    public String[] getSpielstand() throws KommException {
        ...
        this.senden("GSS");
        /*
         * try { xyz = wait2(in.readLine()); } catch (IOException ioe) { throw
         * new KommException
         * ("getspielstand: Fehler beim Lesen (IOException mit Message: "
         * +ioe.getMessage()); }
         */
        String rein = this.einlesen();
        xyz = wait2(rein);
        ...
        if (xyz.typ == ClientAntwort.SPIELSTAND) {
            /*
             * if (xyz.ok==true) return null; else return xyz.namen;
             */
            return xyz.namen;
    ...
    /**
     * Info-Request zur Abfrage des sogenannten Spielstatus. Sie gibt f&uuml;r
     * jeden Bot ein Statusobjekt zur&uuml;ck, das dessen Namen, seine bisher
     * ausgewerteten Registerinhalte und die aktuelle Auswertungsphase als
     * Attribute besitzt. Falls gerade nicht ausgewertet wird, wird null
     * zur�ckgegeben.
     * 
     * @exception KommException
     *                Tritt beim Parsen ein Fehler auf (z.B. wegen falsch
    ...
    public Status[] getSpielstatus() throws KommException {
        ...
        xyz = wait2(rein);
        /*
         * } catch (IOException ioe) { throw new KommException
         * ("IOException bei getSpielstatus; Message: "+ioe.getMessage()); }
         */
        if (xyz.typ == ClientAntwort.SPIELSTATUS) {
            ...
            if (xyz.typ != 0)
                throw new KommException("Falsche Antwort bei getSpielStatus: habe ClientAntwort vom Typ " + xyz.typ
                                + " erhalten");
            else
    ...
    public int getTimeOut() throws KommException {
        ...
        /*
         * } catch (IOException ioe) { throw new KommException
         * ("IOException bei getTimeOut (lesen von 'in')"); }
         */
        com = this.einlesen();
        xyz = wait2(com);
    ...
    /**
     * Gets either the position of the robot (if RobPos==true) or the dimensionj
     * of the board (if RobPos== false)
     * 
     * @param name
     *            WILL BE IGNORED
     */
    private Location fetchOrt(String name, boolean RobPos) throws KommException {
        ...
        try {
            ...
            throw new KommException(method + " warf StringIndexOutOfBoundsException(Inhalt:" + sioob.getMessage()
                            + "); Ursache: vermutlich falsch aufgebaute Antwort vom Server");
        }
        catch (NumberFormatException nfe) {
            CAT.error(nfe.getMessage(), nfe);
        ...
        // catch (IOException ioe) {
        // throw new KommException
        // (method+" warf eine IOException; Message: "+ioe.getMessage());
        // }
        catch (Exception sonstige) {
    ...
    /**
     * Diese Methode parst den Programmkartenteil bei getRobStatus (URL-)decodes
     * the names
     */
    public static Status getStatusRegs(String in) throws KommException {
        ...
        back.robName = Encoder.commDecode(new String(in.substring(1, kommaName)));
        in = in.substring(kommaName + 1); // nur noch die Karten und die
                                          // Klammerzu
        ...
        while (in.length() > 4) {
            ...
            if (i < ks) {
                int komma = in.indexOf(',');// das Komma zwischen Aktion und
                                            // Prioritaet
                ...
                if ((komma != -1) && (klauf != -1) && (klazu != -1)) {
                    ...
                    try {
                        ...
                        throw new KommException("NumberFormatException bei KC.getStatusRegs; Message: "
                                        + nfe.getMessage());
                    }
    ...
    private static Card[] getRegister(String str) throws KommException {
        ...
        while (active.length() > 4) {
            ...
            try {
                ...
                throw new KommException("getRegister: NumberFormatException bei Parsen der Kartenprioritaet: Message: "
                                + nf2.getMessage());
            }
            back[register - 1] = Deck.get(prioritaet); // Card eingeteilt
            // jetzt String aktualisieren:

            active = active.substring(klzu + 3); // "1,PK(M1,123))(" abschneiden
                                                 // KLZU+1
    ...
    /**
     * Gibt die Position (in com) hinter der ')' des ersten Vorkommens von
     * ")("zurueck; gibt es keinen substring ")(", so wird -1 zurueckgegeben.
     */
    ...
    /**
     * Hilfsmethode fuer GetRobStatus-Antwort. Liefert den String zurueck, der
     * den ersten Bot betrifft: Bekommt die Methode
     * "(name1,PK(M1,123))(name2,))" uebergeben,so gibt sie "(name1,PK(M1,123))"
     * zurueck. Der uebergebene String wird nicht veraendert.
     */
