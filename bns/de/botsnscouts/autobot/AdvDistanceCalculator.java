...
public class AdvDistanceCalculator extends DistanceCalculator {
    ...
    private AdvDistanceCalculator(Board board) {
        super(board, false);
        if (distances == null) {
            distances = new int[board.getFlags().length][board.getSizeX() + 2][board.getSizeY() + 2][4];
            CAT.debug("new distances Array: " + distances.length + "x" + distances[0].length + "x"
                            + distances[0][0].length + "x" + distances[0][0][0].length);
        }
    }
    ...
    private void preCalculate(int[][][] dist, int x, int y, int facing) {
        ...
        // properly.
        if (!hasNorthWall(x, y))
            checkSquare(floor(x, y + 1), x, y + 1, facing, dist, x, y, Directions.SOUTH, Directions.NORTH);

        if (!hasSouthWall(x, y))
            checkSquare(floor(x, y - 1), x, y - 1, facing, dist, x, y, Directions.NORTH, Directions.SOUTH);

        if (!hasEastWall(x, y))
            checkSquare(floor(x + 1, y), x + 1, y, facing, dist, x, y, Directions.WEST, Directions.EAST);

        if (!hasWestWall(x, y))
            checkSquare(floor(x - 1, y), x - 1, y, facing, dist, x, y, Directions.EAST, Directions.WEST);
    }

    private void checkSquare(Floor target, int x, int y, int facing, int[][][] dist, int oldx, int oldy,
                    int good_direction, int bad_direction) {
        if (!target.isPit()) {
            if (target.isBelt()) {
                if (target.getBeltDirection() == good_direction)
                    // Belt in coorect direction
                    relax(dist, x, y, facing, oldx, oldy, facing, BELT_COST_PRO[target.getBeltSpeed() - 1]);
                else
                    if (target.getBeltDirection() == bad_direction)
                        // Belt in opposite direction
                        relax(dist, x, y, facing, oldx, oldy, facing, BELT_COST_CONTRA[target.getBeltSpeed() - 1]);
                    else
    ...
    private void relax(int[][][] dist, int x, int y, int facing, int oldx, int oldy, int oldfacing, int cost) {
    ...
    private static Map<Board, AdvDistanceCalculator> uniqueInstances = new HashMap<Board, AdvDistanceCalculator>();
