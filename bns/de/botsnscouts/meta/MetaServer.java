...
public class MetaServer {
    ...
    /**
     * Currently announced games. Each entry is a XMLElement.
     */
    private Vector<XMLElement> games = new Vector<XMLElement>();
    ...
    /**
     * Request to announce a game. Will only be fullfilled after checking that the named server is really a BNS server and that it is accessible.
     * 
     * @param request
     *            XMLElement describing the game that shall be announced.
     * @return answer to be send back to the client, either ok or failure.
     */
    XMLElement announce(XMLElement request) {
    ...
    /**
     * Check wether there is really a BNS server running at the given address. And wether this address is accessible via the internet.
     * 
     * Implementation note: Code from KommClient is not reused on purpose. because it sucks. Additionally, the coupling to BNS classes shall be low
     * because maybe this code will serve as example for implementing the meta server in another programming
     * language.
     * 
     * @param host
     *            hostname that is said to host a BNS server
     * @param port
    ...
    XMLElement query(@SuppressWarnings("unused") XMLElement request) {
        debug("query received.");
        queries++;
        // TODO: (?) Synchronization
        XMLElement list = new XMLElement();
        list.setName("games");
        for (Iterator<XMLElement> it = games.iterator(); it.hasNext();) {
    ...
    XMLElement stat(@SuppressWarnings("unused") XMLElement request) {
        debug("stat received");
        statRequests++;
