...
class ServerAusgabeThread extends BNSThread implements Waitable
{
    ...
    /** The thing is, this Thread  MUST NOT call deleteOutput while it holds its own lock
     *  - which it does most of the time
     * => if we receive a deregistration message, we will throw and catch this exception to leave the 
     *    "synchronized(this)"
     *   
     *   Reason: deleteOutput() requires the lock of the Server's "ausgabeThread"-Vector which is very
     * likely owned by "MessageThread" during phase evaluation; and MessageThread will want to call
     *   our synchronized "sendMsg()"-method one or more times before releasing the lock..
     * => DEADLOCK 
     * 
     */ 
